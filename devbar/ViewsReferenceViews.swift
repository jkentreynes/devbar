...
struct CronExpressionParserView: View {
    @State private var cronExpression: String = "0 12 * * *"
        ...
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 8) {
                    Label("Cron Expression", systemImage: "calendar.badge.clock")
                        .font(.subheadline)
                        .fontWeight(.medium)
                    
                    TextField("Enter cron expression", text: $cronExpression)
                        .font(.system(.body, design: .monospaced))
                        .textFieldStyle(.plain)
                        .padding(8)
                        .background(Color(nsColor: .textBackgroundColor))
                        .cornerRadius(6)
                        .onChange(of: cronExpression) { _, _ in parseCron() }
                }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    CronExample(expression: "0 0 * * *", description: "Every day at midnight")
                    CronExample(expression: "0 12 * * *", description: "Every day at noon")
                    CronExample(expression: "0 0 * * 0", description: "Every Sunday at midnight")
                    CronExample(expression: "0 0 1 * *", description: "First day of every month")
                }
                .font(.caption)
            }
            .padding()
    ...
    func parseCron() {
        ...
        guard parts.count == 5 else {
            explanation = "Invalid cron expression. Format: minute hour day month weekday"
            return
        }
        ...
        if minute == "*" && hour == "*" && day == "*" && month == "*" && weekday == "*" {
            ...
            result += "every hour at minute \(minute)"
        } else if day == "*" && month == "*" && weekday == "*" {
            result += "daily at \(hour):\(minute)"
        } else {
            result += "at custom schedule: \(cronExpression)"
        }
